# Council: Optimal README structure for devlog-mcp

**Date:** 2026-05-26
**Query:** How should the devlog-mcp README be structured to convert agent developers — examples-first vs moat-first — and which examples prove memory value?
...
- ADD two moat-proof examples right after the transcript:
  - `devlog_feedback_query` → per-tool success_rate driving routing (Affective layer).
  - `devlog_entity_graph` with `as_of` → bi-temporal fact (valid_from/valid_to, superseded_by).
